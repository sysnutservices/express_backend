...
function main() {
    ...
    mask[2 * W + 3] = 255; // (3,2)
    mask[5 * W + 7] = 255; // (7,5)
    const bbox = (0, localSegmentation_1.computeBBoxFromMask)(mask, W, H, 127);
    assert_1.default.deepStrictEqual(bbox, { left: 3, top: 2, width: 5, height: 4 });
    // 2. No pixel above threshold -> null, not a crash or a bogus 0-sized box.
    assert_1.default.strictEqual((0, localSegmentation_1.computeBBoxFromMask)(new Uint8Array(W * H), W, H, 127), null);
    ...
    binary[8 * W + 8] = 1;
    const keep = (0, localSegmentation_1.largestComponentMask)(binary, W, H);
    assert_1.default.strictEqual(keep[0 * W + 0], 1, "big blob pixel should be kept");
    assert_1.default.strictEqual(keep[2 * W + 2], 1, "big blob pixel should be kept");
    assert_1.default.strictEqual(keep[8 * W + 8], 0, "disconnected small blob must be discarded");
    assert_1.default.strictEqual(keep.reduce((a, b) => a + b, 0), 9, "only the 3x3 blob's 9 pixels survive");
    ...
    const dilated = (0, localSegmentation_1.boxMorph)(shape, W, H, 1, true);
    const closed = (0, localSegmentation_1.boxMorph)(dilated, W, H, 1, false);
